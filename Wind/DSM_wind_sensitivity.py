# -*- coding: utf-8 -*-
"""
Created on Mon Apr 12 14:52:25 2021

@author: jvano
"""

"""
Sensitivity analysis Wind: increased MI efficiency

The structural materials include concrete, steel, plastic, glass/carbon composites, aluminium, chromium, 
copper, iron, manganese, molybdenum, nickel and zinc. These materials are characterised by a moderate reduction 
in material intensity. In particular, the values in 2050 are equal to 80%, 90% and 100% of the current values 
in the LDS, MDS and HDS, respectively. 

The technology-specific materials include boron, dysprosium, neodymium, praseodymium and terbium. 
For these materials, the following hypothetical situations for material intensity have been considered: 
an annual 5% reduction in the LDS, an annual 2% reduction in the MDS and a constant level of material intensity in the HDS. 
The resulting value in 2050 is about one fifth of the current value in the LDS and half of the current value in the MDS, 
respectively. 
...
# mass% Nd = 20-31% Viebahn et al. (2015) // 23,95% in Munchen et al. (2018)
# mass% Dy = 2-5% Viebahn et al. (2015) // 0.98% in Munchen et al. (2018)
...
#%% Sensitivity analysis lower MI

years = {'year': np.arange(1982, 2051, 1)}
df = pd.DataFrame(years)

# crm is decrease in MI for critical metals (NdFeB magnets)
# struct is decrease in MI for structural metals (list struct_mat)

for index, year in df.iterrows():
    if df.loc[index,'year'] <= 2018:
        df.loc[index,'crm']=1
        df.loc[index, 'struct']=1
    else:
        df.loc[index, 'crm']=df.loc[index-1,'crm']*0.95
        df.loc[index,'struct']=df.loc[index-1,'struct']-0.00625
        
        
struct_mat = ['steel', 'iron', 'copper', '_al', 'cast_iron', 'concrete']
tech_mat = ['NdFeB', 'Yt']


#%% Section 4: Dynamic Stock model (sensitivity version)       
...
files = [low_on, low_off, mid_on, mid_off, high_on, high_off]

# multiply inflows with increase MI values

for file in files:
    file['tech_sens']= df['crm']
    file['struct_sens']=df['struct']                   
    
    # New fast approach, same results:            
    for columnName in file:
        if any(ele in columnName for ele in struct_mat):
            print(columnName)
            file[columnName] = file[columnName]*file['struct_sens']     
        if any(ele in columnName for ele in tech_mat):
            print(columnName)
            file[columnName] = file[columnName]*file['tech_sens']           
...
mid_off, high_off],['low','mid','high']):                                       
  ...
  with pd.ExcelWriter('in-out-stock-{0}-scenario_s.xlsx'.format(excel)) as writer:
